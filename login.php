...
<html>
    <head>
						...
						success:function(data) 
						{
							...
							if(data==1)
							{
								...
								setTimeout(function(){//两秒后跳转
								    location.href = "./user/index.php";//PC网页式跳转
								},2000);
							}
							else if(data==0)
							{
		...
		<script>
			$(function(){
				/*仿刷新：检测是否存在cookie*/
				if($.cookie("captcha_login")){
					var count = $.cookie("captcha_login");
					...
					var resend = setInterval(function(){
						...
						if (count > 0){
							...
							document.getElementById("sendcode").value=count + "秒后重获";
							$.cookie("captcha_login", count, {path: '/', expires: (1/86400)*count});
						}else {
				...
				if($.cookie("email_login"))
				{
					document.getElementById("username").value=$.cookie("email_login");
				}
				...
				$('#sendcode').click(function(){
					...
					$.cookie("email_login", email, {path: '/', expires: (1/86400)*count});
					if(email=="")
					{
						layer.msg("请输入用户名(邮箱)");
					}
				...
				<div class="baobao-width-406080 baobao-x-center" style="float: none;padding-top:10px;">
					<div class="baobao-contain baobao-font-title" style="diaplay:inline-block;height:50px;<?php echo background_image_2(90,$colors['87'],$colors['86'],$colors['87']);?>border-radius:10px 10px 0% 0%;">
						<div style="display:inline-block; *display:inline; zoom:1;">
							<div style="float:left;">
									<span class="baobao-span iconfont icon-confirm"></span>
							</div>
							<div style="float:left;">
									<span class="baobao-span" >影小薯博客系统登录</span>
							</div>
						</div>
					</div>
					<hr>
					<form name="login" action="##" method="post" class="baobao-form">
						...
						<div class="baobao-inline" style="height:50px;background-color:#1A94E6;">
						    <?php if($conf['yzm_login']=="1"){ ?>
							<font style="color:#fff;line-height:50px;font-weight:bold;" >点击验证码更换</font>
							<span  for="name" style="float:right;"><img  style="margin:0px;" height="50" id="codeimg" class="baobao-yzm" src="../include/code.php?act=login&r=<?php echo time();?>" onclick="this.src='../include/code.php?act=login&r='+Math.random();" title="点击更换验证码"></span>
							<?php }else{?>
						...
						<?php }?>
						<div class="baobao-inline" style="height:50px;background-color:#1A94E6;">
							<div style="float:left;width:100%;"><input type="button" id="submit" name="submit"  style="height:100%;font-weight:bold;font-size:18px;color:<?php echo $colors['89'];?>;<?php echo background_image_2(90,$colors['92'],$colors['91'],$colors['92']);?>border:none;width:100%;cursor:pointer;" value="登录"></div>
						</div>
					</form>
					<hr>
					<div class="baobao-contain baobao-font-title" style="float:none;<?php echo background_image_2(90,$colors['87'],$colors['86'],$colors['87']);?>border-radius:0% 0% 10px 10px;font-size:10px;">
						<a href="./find.php"><span style="color:<?php echo $colors['88'];?>;">忘记密码</span></a><a href="./reg.php"><span style="color:<?php echo $colors['88'];?>;margin-left:30px;">没有账号,前往注册</span></a>
					</div>
				</div>
			</div>
		</div>

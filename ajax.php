	...
	if($flag==1)
	{
		...
		{   
			...
			else
			{
    			...
    			if($method=="login")
    			{
            		...
            		setcookie('code_login',$code,time()+$time);
    			}
    			elseif($method=="reg")
    			{
        	    	...
        	    	setcookie('code_reg',$code,time()+$time);
    			}
    			elseif($method=="find")
    			{
            		...
            		$_SESSION['do']=0;
            		setcookie('code_find',$code,time()+$time);
    			}
		...
		else if($act=="login")
		{
		    ...
		    if($conf['yzm_login']!="0")
		    {
		        ...
		        if(isset($_COOKIE['code_login']))
		        {
		            $_SESSION['vc_code_login']=$_COOKIE['code_login'];
		        }
		        else if(!isset($_SESSION['vc_code_login']))
		        {
		            $_SESSION['vc_code_login']=0;
		        }
		...
		else if($act=="reg")
		{
		    ...
		    {
		        $yzm=isset($_POST['yzm'])? daddslashes($_POST['yzm']):"";
		        if(isset($_COOKIE['code_reg']))
		        {
		            $_SESSION['vc_code_reg']=$_COOKIE['code_reg'];
		        }
		        else if(!isset($_SESSION['vc_code_reg']))
		        {
		            $_SESSION['vc_code_reg']=0;
		        }
		...
		else if($act=="find")
		{
		    ...
		    else
		    {
    		    ...
    		    {
    		        $yzm=isset($_POST['yzm'])? daddslashes($_POST['yzm']):"";
    		        if(isset($_COOKIE['code_find']))
    		        {
    		            $_SESSION['vc_code_find']=$_COOKIE['code_find'];
    		        }
    		        else if(!isset($_SESSION['vc_code_find']))
    		        {
    		            $_SESSION['vc_code_find']=0;
    		        }

<?php
	...
	if($flag==1)
	{
		...
		{
		     $db = new mysqli($dbconfig['host'],$dbconfig['user'],$dbconfig['pwd'],$dbconfig['dbname'],$dbconfig['port']);
		      $id=isset($_POST['color_id'])? daddslashes($_POST['color_id']):"";
		      $value=isset($_POST['color_value'])? daddslashes($_POST['color_value']):"";
		      $sql =  $db->query("update brj_color set value= '".$value."' where id='".$id."'");
		      if($sql)
		      {
		          echo 1;
		      }
